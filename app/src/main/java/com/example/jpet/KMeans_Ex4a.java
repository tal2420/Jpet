...
/**
 * Created by eliyadaida on 5/31/15.
 */
...
public class KMeans_Ex4a
{
    ...
    private static void kMeanCluster()
    {
        ...
        while(dataSet.size() < TOTAL_DATA)
        {
            ...
            {
                distance = dist(newData, centroids.get(i));  // מחשב את המרחק המינימלי. כלומר למי הנק הזאת הכי קרובה מבין הcluster center
                if(distance < minimum){
                    minimum = distance;
                    cluster = i;
                }
            ...
            for(int i = 0; i < NUM_CLUSTERS; i++)
            {
                ...
                for(int j = 0; j < dataSet.size(); j++)  //עובר כל נק במערך ומחלק לcluster, אם הוא שייך אליו אז הוא מחבר את כל הx והy שלהם.
                {
                ...
                if(totalInCluster > 0){
                    centroids.get(i).X(totalX / totalInCluster);  // אם יש נק שהן בcluster הזה אז הוא לוקח את הx הטוטלי שיצא מקודם ומחלק במס הנק ששיכות לcluster הזה
